...
public class AddWatermarkTriggerByStorageFunction
{
    const string FUNCTION_NAME = "AddWatermarkTriggerByStorage";
    ...
    public static async Task Run([BlobTrigger("images/{name}", Connection = "AzureWebJobsStorage")] Stream myBlob, string name, ILogger log)
    {
        ...
        try
        {
            ...
            var watermarkItem = await GetWatermarkItemByImageBlobNameAsync(name);
    ...
    private static async Task<WatermarkItem> GetWatermarkItemByImageBlobNameAsync(string imageBlobName)
    {
        var cosmosClient = new CosmosClient(WatermarkSampleCosmosDbConnection);
        var cosmosDbService = new CosmosDbService(cosmosClient, WatermarkSampleCosmosDbName, WatermarkSampleCosmosDbContainerName);
        var items = await cosmosDbService.GetByImageBlobName(imageBlobName);
